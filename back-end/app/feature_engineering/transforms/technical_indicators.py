...
class TechnicalIndicatorsTransform:
    ...
    def calculate_sma(df: pd.DataFrame, period: int, column: str = "close") -> pd.Series:
        return TechnicalIndicatorsTransform._by_symbol(df, column, lambda values: values.rolling(window=period).mean())

    @staticmethod
    def calculate_ema(df: pd.DataFrame, period: int, column: str = "close") -> pd.Series:
        return TechnicalIndicatorsTransform._by_symbol(df, column, lambda values: values.ewm(span=period, adjust=False).mean())

    @staticmethod
    def calculate_rsi(df: pd.DataFrame, period: int = 14, column: str = "close") -> pd.Series:
        return TechnicalIndicatorsTransform._by_symbol(df, column, lambda values: pd.Series(talib.RSI(values.to_numpy(dtype=float), timeperiod=period), index=values.index))
    ...
    @staticmethod
    def calculate_avg_price_deviation(df: pd.DataFrame, period: int = 20, column: str = "close") -> pd.Series:
        sma = TechnicalIndicatorsTransform.calculate_sma(df, period, column)
        return (df[column] - sma) / sma

    @staticmethod
    def calculate_bollinger_bands(df: pd.DataFrame, period: int = 20, num_std: int = 2, column: str = "close") -> tuple[pd.Series, pd.Series, pd.Series]:
        middle_band = TechnicalIndicatorsTransform.calculate_sma(df, period, column)
        rolling_std = TechnicalIndicatorsTransform._by_symbol(df, column, lambda values: values.rolling(window=period).std())
        return middle_band + (rolling_std * num_std), middle_band, middle_band - (rolling_std * num_std)
    ...
    def calculate_atr(df: pd.DataFrame, period: int = 14) -> pd.Series:
        ...
        for positions in df.groupby("symbol", sort=False).indices.values():
            ...
            result.iloc[positions] = talib.ATR(candles["high"].to_numpy(dtype=float), candles["low"].to_numpy(dtype=float), candles["close"].to_numpy(dtype=float), timeperiod=period)
        return result
    ...
    @staticmethod
    def calculate_change_percent(df: pd.DataFrame, column: str = "close", period: int = 1) -> pd.Series:
